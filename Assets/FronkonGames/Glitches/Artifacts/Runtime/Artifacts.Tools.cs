...
namespace FronkonGames.Glitches.Artifacts
{
  ...
  public sealed partial class Artifacts
  {
    ...
    private static readonly Artifacts[] NoEffects = new Artifacts[0];
    ...
    public static Artifacts Instance
    {
      get
      {
        ...
        if (pipelineAsset != null)
        {
          ...
          {
            if (scriptableRendererData.rendererFeatures[i] is Artifacts)
              return scriptableRendererData.rendererFeatures[i] as Artifacts;
          }
        }
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    private static Artifacts[] Instances
    {
      get
      {
          ...
          for (int i = 0; i < rendererDataList.Length; ++i)
          {
            if (rendererDataList[i] != null && rendererDataList[i].rendererFeatures.Count > 0)
              foreach (var feature in rendererDataList[i].rendererFeatures)
                if (feature is Artifacts)
                  effects.Add(feature as Artifacts);
          }
