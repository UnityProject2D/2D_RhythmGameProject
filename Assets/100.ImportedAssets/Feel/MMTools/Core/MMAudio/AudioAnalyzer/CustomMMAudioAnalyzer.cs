...
namespace MoreMountains.Tools
{
	...
	/// </summary>
	[AddComponentMenu("More Mountains/Tools/Audio/Custom MM Audio Analyzer")]
	public class CustomMMAudioAnalyzer : MonoBehaviour
	{
        ...
        public float[] ExternalSpectrumData;
        protected virtual IEnumerator Analyze()
		{
			while (true)
			{
                if (ExternalSpectrumData != null)
                {
                    RawSpectrum = ExternalSpectrumData;
                }
                else
                {
                    TargetAudioSource.GetSpectrumData(RawSpectrum, 0, Window);
